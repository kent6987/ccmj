...
var loadCrons = function(server, app) {
  ...
  app.loadConfig(Constants.Reserved.CRONS, path.join(app.getBase(), Constants.FilePath.CRON));
 ...
 */
var afterFilter = function(isGlobal, server, err, msg, session, resp, cb) {
  var fm;
  if(isGlobal) {
    fm = server.globalFilterService;
  } else {
...
var response = function(isGlobal, server, err, msg, session, resp, cb) {
  if(isGlobal) {
    ...
    afterFilter(isGlobal, server, err, msg, session, resp, cb);
  }
};
...
var doForward = function(app, msg, session, routeRecord, cb) {
  ...
  //should route to other servers
    console.log('----------------------------------------routeRecord :'+ JSON.stringify(routeRecord));
  try {
